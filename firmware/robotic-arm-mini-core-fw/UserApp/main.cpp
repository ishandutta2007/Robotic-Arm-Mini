...
PWM pwm(21000, 21000);
// Robot instance
RoboticArmMini roboticArm(&hcan1);
...
void ThreadControlLoopFixUpdate(void* argument)
{
    for (;;)
    {
        ...
        if (roboticArm.IsEnabled())
        {
            // Send control command to Motors & update Joint states
            switch (roboticArm.commandMode)
            {
                case RoboticArmMini::COMMAND_TARGET_POINT_SEQUENTIAL:
                case RoboticArmMini::COMMAND_TARGET_POINT_INTERRUPTABLE:
                case RoboticArmMini::COMMAND_CONTINUES_TRAJECTORY:
                    roboticArm.MoveJoints(roboticArm.targetJoints);
                    roboticArm.UpdateJointPose6D();
                    break;
                case RoboticArmMini::COMMAND_MOTOR_TUNING:
                    roboticArm.tuningHelper.Tick(10);
                    roboticArm.UpdateJointPose6D();
                    break;
            }
        } else
...
void ThreadControlLoopUpdate(void* argument)
{
    for (;;)
    {
        roboticArm.commandHandler.ParseCommand(roboticArm.commandHandler.Pop(osWaitForever));
    }
}
...
void ThreadOledUpdate(void* argument)
{
    ...
    for (;;)
    {
        ...
        oled.printf(">%3d|%3d|%3d|%3d|%3d|%3d",
                    (int) roundf(roboticArm.currentJoints.a[0]), (int) roundf(roboticArm.currentJoints.a[1]),
                    (int) roundf(roboticArm.currentJoints.a[2]), (int) roundf(roboticArm.currentJoints.a[3]),
                    (int) roundf(roboticArm.currentJoints.a[4]), (int) roundf(roboticArm.currentJoints.a[5]));

        oled.drawBox(40, 35, 128, 24);
        oled.setFont(u8g2_font_6x12_tr);
        oled.setDrawColor(0);
        oled.setCursor(42, 45);
        oled.printf("%4d|%4d|%4d", (int) roundf(roboticArm.currentPose6D.X),
                    (int) roundf(roboticArm.currentPose6D.Y), (int) roundf(roboticArm.currentPose6D.Z));
        oled.setCursor(42, 56);
        oled.printf("%4d|%4d|%4d", (int) roundf(roboticArm.currentPose6D.A),
                    (int) roundf(roboticArm.currentPose6D.B), (int) roundf(roboticArm.currentPose6D.C));
        oled.setDrawColor(1);
        oled.setCursor(0, 45);
        oled.printf("[XYZ]:");
        oled.setCursor(0, 56);
        ...
        {
            for (int i = 1; i <= 6; i++)
                buf[i - 1] = (roboticArm.jointsStateFlag & (1 << i) ? '*' : '_');
            buf[6] = 0;
            oled.printf("[%s] %s", cmdModeNames[roboticArm.commandMode - 1], buf);
        } else
        {
            oled.printf("[%s] %s", cmdModeNames[roboticArm.commandMode - 1], "======");
        }
